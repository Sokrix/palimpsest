---
mode: agent
description: 'Prime — Load vault context at session start'
---

# Prime — Context loading

Loads the vault context at the start of every session.

> `<vault>` = `{{VAULT_PATH}}`

The global vault rules and schema are loaded automatically from `~/.copilot/instructions/palimpsest.instructions.md` — `/prime` does not need to read them.
